...
- Phase 3 authorizes **Script-to-Film only** concept integration through the shared Film Planner: suggestions may read `retrieve_best_recipes()`, explicit Apply may store recipe metadata on the current Film scene and append its positive fragment to that scene's keyframe prompt, and Research more may save only human-approved Phase 2 candidates. Do not use this path in Music Video mode; never write recipe trigger words to Character Bible or auto-map recipe LoRAs into the LoRA Knowledge Base.
- Civitai concept research must stay in the standalone `tools/civitai_concept_researcher/` helper. ComfyUI nodes may only call it, review JSON, and save explicit user-approved candidates through the Concept/Pose Knowledge Base. Never auto-save research results or copy browser sessions/API tokens into the project.
