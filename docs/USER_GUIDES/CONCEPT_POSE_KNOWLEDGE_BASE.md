...
6. If the library has no suitable result, choose **Research more for this concept**. Review the returned Civitai candidates, check only the candidates you approve, and choose **Approve selected and save to local recipes**. The planner immediately refreshes the local suggestions after the save. Safe-only research is on by default; clearing it immediately re-runs the search as adult-allowed and clears any prior candidate checks.
