...
| Native sampler vocabulary and iteration progress | `web/VRGDG_SamplerOptions.js` is the shared fallback; the Builder refreshes the live list from `/vrgdg/workflow_runner/i2v_choices`, and `wizardSnapshot().i2vSamplerOptions` feeds the same list to the Wizard. Scene-video progress uses the shared wait path and reports sampler `current/total` plus `s/it` in both render surfaces. See `SAMPLER_OPTIONS_AND_SCENE_PROGRESS.md`. |
...
| Script-to-Film LoRA Knowledge Base | `script_to_film.lora_knowledge_loras`, optional `style_profile_path`, and per-scene `lora_knowledge_refs`. Builder and Wizard use the same `VRGDG_ScriptToFilmUI.js` modal and the same Film-only metadata/resolution routes; the Character Bible remains identity-only and Music Video is not routed through this feature. See `LORA_KNOWLEDGE_BASE.md`. |
